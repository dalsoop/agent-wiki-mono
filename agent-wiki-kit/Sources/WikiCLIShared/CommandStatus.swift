...
public func runStatus(store: LedgerStore, worldName: String, arguments: [String]) {
    ...
    print("world: \(worldName)  (\(store.root.path))") // allow:debug
    print("객체: head \(heads) · 전체 \(total)" + (indexed ? "" : "  [인덱스 없음 — 전체 스캔]")) // allow:debug
    print("최근 발행: \(latest ?? "(없음)")") // allow:debug
    print("무결성: agent-wiki verify") // allow:debug
}
