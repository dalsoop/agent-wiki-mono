...
public enum AuthorAudit {
    ...
    static func run(root: URL, objects: [LedgerObject]) -> [LedgerStore.Violation]? {
        ...
        guard let committerByID = committerEmailByObjectID(root: root) else {
            return [LedgerStore.Violation(
                id: "authors.json", problem: "author 감사 불가 — 원장이 git 저장소가 아님")]
        }
        ...
        for object in objects where object.ledger >= 2 {  // content-addressed 만
            ...
            if object.author != expected {
                violations.append(LedgerStore.Violation(
                    id: object.id,
                    problem: "author 위조 의심 — 신고 '\(object.author)' vs 커미터 <\(email)> → 기대 '\(expected)'"))
            }
        }
        if unverifiable > 0 {
            FileHandle.standardError.write(Data(
                "(author 감사: 커미터 \(unverifiable)건이 authors.json 에 없어 검증 못 함)\n".utf8))
        }
        return violations
    }
    ...
    public static func resolveAuthorsMap(from root: URL) -> [String: String]? {
