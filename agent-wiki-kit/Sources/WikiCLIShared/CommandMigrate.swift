...
private func writeMigratedObjects(
    _ report: LedgerMigration.Report, objectsDir: URL
) -> Int {
...
private func removeRemappedFiles(_ report: LedgerMigration.Report, objectsDir: URL) {
    var removed = 0
    if let enumerator = FileManager.default.enumerator(
        at: objectsDir, includingPropertiesForKeys: nil, options: [.skipsHiddenFiles]) {
        for case let url as URL in enumerator where url.pathExtension == "md" {
            let name = url.deletingPathExtension().lastPathComponent
            if let newID = report.remap[name], newID != name {
                do {
    ...
    print("구 uuid 파일 삭제 \(removed)") // allow:debug
}

public func runMigrate(store: LedgerStore, arguments: [String]) {
    ...
    print(summary) // allow:debug

    guard apply else {
        print("(dry-run — 실제 쓰기는 --apply. 비파괴로 새 위치에 쓰려면 --to <dir>)") // allow:debug
        return
    }
    ...
    let written = writeMigratedObjects(report, objectsDir: objectsDir)
    print("작성 \(written) 파일 → \(objectsDir.path)") // allow:debug

    if inPlace { removeRemappedFiles(report, objectsDir: objectsDir) }
    print("완료. `verify` 로 무결성 확인 권장.") // allow:debug
}
