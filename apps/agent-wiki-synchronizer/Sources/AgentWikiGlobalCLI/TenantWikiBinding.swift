...
public enum TenantWikiBinding: Sendable {
    public static func activeWorldName() -> String? {
        ...
        let contextURL = home.appendingPathComponent(".config/agent-tenant-isolation/context.json")
        ...
        let ledgerURL = home.appendingPathComponent(".config/agent-tenant-isolation/tenants.json")
        guard let ledgerData = try? Data(contentsOf: ledgerURL) else {
            return "person-\(activeSlug)"
        }
        let ledgerJson: [String: Any]? = {
            do {
                return try JSONSerialization.jsonObject(with: ledgerData) as? [String: Any]
            } catch {
                return nil
            }
        }()
