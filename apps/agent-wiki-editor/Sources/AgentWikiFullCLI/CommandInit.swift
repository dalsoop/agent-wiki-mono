...
func runInit(arguments: [String]) -> Never {
    guard arguments.count >= 2 else { fail(usage) }
    if CLIArgv.isHelpToken(arguments[1]) {
        print(CLILocalization.string("CommandInit.print"))  // allow:debug — CLI 사용법 stdout
        exit(0)
    }
...
func bundledAgentRoleDefinitions() -> [(String, String)] {
    (ResourceBundle.named("AgentWikiStudio_AgentWikiFullCLI").urls(
...
func registerWorld(root url: URL, roleCount: Int) {
    do {
        ...
        try config.save()
        print(CLILocalization.format("CommandInit.print-2", url.path, roleCount))  // allow:debug — CLI 결과 stdout
    } catch {
