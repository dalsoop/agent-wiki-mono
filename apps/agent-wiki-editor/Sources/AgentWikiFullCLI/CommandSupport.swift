...
usage: \(tool) [--as <author>] [--world <name>] <command> [args]
  ...
  publish [--title <제목>] [--type <유형>] [--origin <url>] [--tag <tag>|--alias <별칭>]...
          [--cite <id> [rel]]... [--observes <event-id>]... [--supersedes <id>] [--retracts <id>]
          [--batch <id>] [--domain <d> --kind <k> --knowledge <n>
          --classification-reason <근거>] [--allow-unclassified]
                                  stdin 본문으로 새 객체 발행. 3축+근거를 주면 선별 객체도 같은 batch에 자동 발행
  publish --kind scene-evidence --of <decision-id> [--title <제목>]
                                  결정 객체의 현장 근거를 독립 객체로 발행(본문 of: 역링크 + cite rel of)
  ...
  path <id접두어> <id접두어>        두 객체 사이 인용 경로 추적(BFS) — CLILocalization.string("CommandSupport.string")
  ...
  task knowledge-candidate <task> CLILocalization.string("CommandSupport.string-2")
  ...
  world use <이름>|add <이름> <경로> [--display <표시명>]   세계관 전환·등록(표시명은 사람용)
...
func printJSON(_ value: some Encodable) {
    ...
    guard let data = try? encoder.encode(value), let text = String(data: data, encoding: .utf8) else {
        fail(CLILocalization.string("CommandSupport.string-3"))
    }
    print(text)
}

/// 인덱스가 있으면 열고 신선도 보장 후 반환(읽기 가속). 없으면 nil → 호출측이 스캔 폴백.
func freshIndex(_ store: LedgerStore) -> LedgerIndex? {
...
func syncIndexAfterWrite(_ store: LedgerStore) {
    let p = store.root.appendingPathComponent("state/index.db")
...
func loadObject(_ store: LedgerStore, id: String) -> LedgerObject? {
    if let index = freshIndex(store), let path = index.pathFor(id: id) {
        do {
            let text = try String(contentsOf: URL(fileURLWithPath: path), encoding: .utf8)
            if let obj = LedgerObject.parse(text)?.object {
                return obj
            }
        } catch {
            _ = error
...
func resolve(_ store: LedgerStore, _ prefix: String) -> LedgerObject {
    ...
    if FileManager.default.fileExists(atPath: indexPath.path) {
        ...
        if ids.count > 1 {
            ...
            fail(CLILocalization.format("CommandSupport.string-4", ids.count, sample))
        }
        // ids 비었으면(인덱스 지연 등) 스캔 폴백으로.
    }
    let objects = store.scan()
    guard let object = store.find(objects, idPrefix: prefix) else {
        fail(CLILocalization.format("CommandSupport.string-5", prefix))
    }
    return object
}
