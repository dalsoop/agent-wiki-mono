...
private func applyScan(
    ...
    try store.save(reg)
    // LedgerConfig 에도 경로 동기화 (resolveWorld GUI 목록) — 죽은 건 안 지움(사용자 확인 후).
    var config = LedgerConfig.load()
    var worlds = config.effectiveWorlds
    for c in candidates {
        if !worlds.contains(where: { $0.rootPath == c.rootPath || $0.name == c.name }) {
            worlds.append(LedgerWorld(name: c.name, rootPath: c.rootPath))
        }
    }
...
private func runFleetRegister(arguments: [String]) {
    ...
    do {
        ...
        worlds.append(LedgerWorld(name: name, rootPath: abs))
        config.worlds = worlds
        try? config.save()
    } catch {
        fail("\(error)")
