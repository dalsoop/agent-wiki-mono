...
func runReview(store: LedgerStore, author: String, arguments: [String]) {
    ...
    do {
        ...
        let review = try store.publish(
            author: author, title: CLILocalization.format("CommandReview.title", label, title), type: "review",
            body: verdict.body(), cites: [.init(id: digest.id, rel: ReviewVerdict.rel)])
        print(CLILocalization.format("CommandReview.print", label, String(review.id.prefix(8)), String(digest.id.prefix(8))))
        if decision == .accept {
            let concept = try store.publish(
                author: author, title: CLILocalization.format("CommandReview.title-2", title), type: "concept",
                body: digest.body, cites: [.init(id: digest.id, rel: "establishes")])
            print(CLILocalization.format("CommandReview.print-2", String(concept.id.prefix(8)), title))
        } else {
            print(CLILocalization.string("CommandReview.print-3"))
        }
    } catch { fail("\(error)") }
}
