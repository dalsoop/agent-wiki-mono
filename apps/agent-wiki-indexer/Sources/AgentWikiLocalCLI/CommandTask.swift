...
func runTask(
    ...
    switch sub {
    case "new":
        runTaskNew(service: service, arguments: arguments)
    case "handoff":
        runTaskHandoff(service: service, arguments: arguments)
    case "bind":
        runTaskBind(service: service, arguments: arguments)
    case "verify":
        runTaskVerify(service: service, author: author, arguments: arguments)
    case "adapter":
        runTaskAdapter(store: store, author: author, repository: repository, arguments: arguments)
    case "capsule", "context":
        runTaskCapsule(store: store, repository: repository, sub: sub, arguments: arguments)
    case "done":
        runTaskDone(service: service, arguments: arguments)
    case "knowledge-candidate":
        runTaskKnowledgeCandidate(service: service, arguments: arguments)
    case "list":
        runTaskList(store: store, repository: repository, arguments: arguments)
    case "chain":
        runTaskChain(store: store, arguments: arguments)
    default:
        fail(
            "task 하위명령: adapter | capsule|context | new | handoff | bind | verify | "
            + "done --verification <receipt> | knowledge-candidate | "
            + "list [--open] [--json] | chain <id>"
        )
    }
}
...
private func runTaskNew(service: RepositoryTaskService, arguments: [String]) {
...
private func runTaskHandoff(service: RepositoryTaskService, arguments: [String]) {
    guard arguments.count >= 3 else {
        fail("사용법: task handoff <task> --to <에이전트> [메모=stdin]")
    }
...
private func runTaskBind(service: RepositoryTaskService, arguments: [String]) {
    guard arguments.count >= 3 else {
        fail(
            "사용법: task bind <task> --runtime-task <id> --dispatch <id> "
            + "[--knowledge <id[,id...]>] [--rule <text>] [--json]"
        )
    }
...
private func runTaskVerify(service: RepositoryTaskService, author: String, arguments: [String]) {
    guard arguments.count >= 3 else {
    ...
    do {
        ...
        if arguments.contains("--json") {
            struct Output: Encodable {
                let receiptObjectId: String
                let receipt: RepositoryAgentVerificationReceipt
            }
...
private func runTaskAdapter(
    ...
    do {
        ...
        switch event.event {
        case .task, .dispatch:
            try handleAdapterTaskOrDispatch(
                store: store, author: author, repository: repository,
                event: event, task: task, objects: objects)
        case .workerDone:
            try handleAdapterWorkerDone(
                store: store, author: author, repository: repository,
                event: event, task: task, objects: objects)
        case .checker:
            try handleAdapterChecker(
...
private func handleAdapterTaskOrDispatch(
    store: LedgerStore,
    author: String,
    repository: RepositoryIdentity?,
    event: RepositoryAgentAdapterEvent,
    task: LedgerObject,
...
private func handleAdapterWorkerDone(
    store: LedgerStore,
    author: String,
    repository: RepositoryIdentity?,
    event: RepositoryAgentAdapterEvent,
...
private func handleAdapterChecker(
    store: LedgerStore,
    author: String,
    event: RepositoryAgentAdapterEvent,
    task: LedgerObject,
...
private func runTaskDone(service: RepositoryTaskService, arguments: [String]) {
    guard arguments.count >= 3 else {
        fail("사용법: task done <task> --verification <receipt-id> [결과=stdin]")
    }
...
private func runTaskKnowledgeCandidate(service: RepositoryTaskService, arguments: [String]) {
...
private func runTaskList(
    ...
    if arguments.contains("--json") {
        printTaskListJSON(graph: graph, repoId: repository?.repoId, openOnly: openOnly)
        return
    }
    for task in graph.tasks {
        ...
        print(CLILocalization.format("CommandTask.print-2", String(mark), String(task.id.prefix(8)), String(task.title ?? CLILocalization.string("CommandTask.noTitle")), String(who), String(task.author), String(gate)))
    }
    if !openOnly {
...
private func printTaskListJSON(graph: LedgerTaskGraph, repoId: String?, openOnly: Bool) {
    let contract = RepositoryTaskListContract(graph: graph, repoId: repoId)
    guard openOnly else {
        printJSON(contract)
        return
    }
...
private func runTaskChain(store: LedgerStore, arguments: [String]) {
