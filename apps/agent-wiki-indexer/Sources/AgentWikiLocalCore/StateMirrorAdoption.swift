...
///
/// GUI 앱은 핵심 모델 요약을 `~/.swift-app-state/agent-wiki-local.json` 으로 게시해
/// 에이전트·스크립트가 스크린샷 없이 `swift-app-router state agent-wiki-local` 로 읽는다.
/// 스캐폴드는 앱 의미를 지어내지 않는다 — 최소 필드(status/generatedAt)만 게시하고,
/// 앱 고유 요약 필드와 호출 지점 연결은 앱 소유자가 채운다.
public enum StateMirrorAdoption {
    /// StateMirror 파일 키 — `~/.swift-app-state/agent-wiki-local.json`.
    public static let appName = "agent-wiki-local"

    public struct State: Codable, Sendable {
        public var status: String
        public var generatedAt: Date
        public var worldKind: String

        public init(
            status: String,
            generatedAt: Date = Date(),
            worldKind: String = "repo-wiki"
        ) {
            self.status = status
