...
/// 한 상황의 논리 사실. 문장이 평가 단위다. 패턴은 수로 바꾸지 않는다.
...
public struct LogicalHormoneDNA: Sendable, Codable, Equatable {
    ...
    /// 직전 패턴을 예측으로 두고, 마지막 패턴을 대조한다. 패턴을 RNA로 바꾸지 않는다.
...
/// 티어 목록의 한 칸. 티어 이름 상수는 없다. 자리와 문장만 있다.
...
/// 쌓인 논리 문장이 invert로 닫히면 그게 호르몬이다. 분자 이름은 없다.
...
/// 논리 호르몬을 만들고 월드 항상성을 invert로 되돌린다.
...
public struct LogicalHormoneWorld: Sendable, Equatable {
    ...
    /// 관측된 RNA가 있는 유전자만 invert한다. 빈 월드에 항상성을 만들지 않는다.
    ...
    /// 문장을 각성 예측 순으로 세우고, 이웃 평균보다 자기 예측에 가까우면 그 자리에 둔다.
    ...
    /// 관측이 쌓인 문장만 호르몬이 된다. 이웃 평균이 자기 예측보다 관측을 잘 설명하면 같은 호르몬이다.
